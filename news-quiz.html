...
async function removeProspect(email) {
  ...
  try {
    await fetch('/api/prospects/' + encodeURIComponent(email), { method: 'DELETE' });
    loadProspects();
...
function postAnswerDist(date, qIdx, correct) {
  if (archiveMode) return;
  fetch('/api/answers', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ date, answers: [{ qIdx, correct }], playerName: state.playerName || '' })
  }).catch(err => {
...
async function saveProgressToServer(completedOverride = false) {
  console.log('saveProgressToServer called', state.playerName, state.todayKey, state.sessionScore, state.answers);
  if (archiveMode) return;
  ...
  console.log('posting progress payload', progress);
...
function dismissFirstTimeOverlay() {
  ...
  lsSet('dnq_seen_intro', '1');
  startQuiz(window._pendingQuiz, window._pendingQuizOptions);
...
if (!lsGet('dnq_seen_intro') && !resumed) {
    window._pendingQuiz = quiz;
...
function selectAnswer(idx) {
 ...
 // Save running score to server after every answer
  if (!state.isReplay) {
    ...
    if (!state.isReplay) {
       postAnswerDist(state.todayKey, state.currentQ, correct);
       queueProgressSave();
    }
...
function selectBonus(idx) {
     if (bonusAnswered) return;
      ...
      if (!state.isReplay) {
          postAnswerDist(state.todayKey, 5, correct);
      }
...
async function renderLeaderboard(tab) {
  ...
  // Server is the sole source of truth — all players see the same leaderboard
  let players = {};
  try {
  ...
  let rows = Object.entries(players).map(([key, p]) => ({
    ...
    score: tab === 'alltime' ? (p.allTime || 0) : ((p.dailyScores || {})[today] || 0),
  ...
  if (today && today.questions) {
    document.getElementById('published-preview').innerHTML =
      ...
      today.questions.map((q,i) => {
        ...
        return `<div class="q-review-card" style="cursor:pointer;" onclick="jumpToArchiveQuestion('${todayStr()}', ${i})" title="View full question & answers in archive"><div class="q-meta"><span>Q${i+1}${diffBadge}</span></div><div class="q-preview">${escHtml(q.question)}${srcLink}</div></div>`;
      }).join('');
  } else {
...
function enforceQuizRules(questions) {
  ...
  return deduped.map(q => {
    ...
    // Mismatch detection: check explanation matches marked correct option
    if (fixed.explanation && fixed.options && fixed.options.length === 4) {
      const markedOption = fixed.options[fixed.correctIndex] || '';
      const expl = fixed.explanation.toLowerCase();
      const words = markedOption.toLowerCase().split(/\s+/).filter(w => w.length > 3);
      const markedScore = words.filter(w => expl.includes(w)).length;
      if (markedScore < 2) {
        let bestIdx = fixed.correctIndex;
        let bestScore = markedScore;
        fixed.options.forEach((opt, i) => {
          if (i === fixed.correctIndex) return;
          const optWords = opt.toLowerCase().split(/\s+/).filter(w => w.length > 3);
          const score = optWords.filter(w => expl.includes(w)).length;
          if (score > bestScore) { bestScore = score; bestIdx = i; }
        });
        if (bestIdx !== fixed.correctIndex) {
          console.warn('[CorrectIndex] Mismatch on Q, correcting ' + fixed.correctIndex + ' -> ' + bestIdx);
          fixed.correctIndex = bestIdx;
        }
      }
    }
      ...
      entries
        ...
        .forEach(([playerKey, p]) => {
          ...
          for (let i = 0; i <= 5; i++) {
            ...
            qCells += `<td style="padding:4px 8px;text-align:center;color:${color};font-family:monospace;">${cell}</td>`;
          }
