...
// ── ONE-TIME: rebuild `bylines` after the Aug 2026 data-loss incident ──
// `scores` was recovered from a volume backup + merge, but `bylines` is
// derived from `progress`, which prunes after ~5 days — it couldn't
// self-heal the way `scores` did, so most of March–Aug 27 is missing from
// it even though the underlying perfect-150 facts still exist in `scores`.
// This backfills any date NOT already present in `bylines` (additive only,
// never touches an existing date) using:
//   - scores.dailyScores === 150 for every date outside the Aug 23-27 gap
//     (real, non-estimated data — same criteria the app already uses)
//   - a hardcoded confirmed list for the Aug 23-27 gap itself, since some
//     estimated gap-day values coincidentally equal 150 and are NOT real
//     bylines
app.post('/api/admin/rebuild-bylines', async (req, res) => {
  const adminToken = process.env.ADMIN_TOKEN || 'admin';
  if (req.headers['x-admin-token'] !== adminToken) return res.status(403).json({ error: 'Forbidden' });
  try {
    const GAP_CONFIRMED = {
    ...
    // Collect every date any player has a dailyScore for, plus the gap dates.
    ...
    const today = easternToday();
    let datesAdded = 0, awardsAdded = 0;
    ...
    for (const date of allDates) {
      ...
      if (GAP_DATES.has(date)) {
        for (const key of (GAP_CONFIRMED[date] || [])) {
          const p = scores[key];
          awarded[key] = { displayName: (p && p.displayName) || key, awardedAt: `${date}T12:00:00.000Z`, backfilled: true };
        }
      } else {
        for (const [key, p] of Object.entries(scores)) {
          if ((p.dailyScores || {})[date] === 150) {
            awarded[key] = { displayName: p.displayName || key, awardedAt: `${date}T12:00:00.000Z`, backfilled: true };
          }
        }
      }

      bylines[date] = awarded; // written even when empty, matching awardBylinesForDate's semantics
      datesAdded++;
      awardsAdded += Object.keys(awarded).length;
    }

    await setKey('bylines', bylines);
    console.log(`[RebuildBylines] Added ${datesAdded} date(s), ${awardsAdded} award(s).`);
    res.json({ ok: true, datesAdded, awardsAdded });
  } catch (e) {
    console.error('[RebuildBylines] error:', e.message);
    res.status(500).json({ error: e.message });
  }
});
